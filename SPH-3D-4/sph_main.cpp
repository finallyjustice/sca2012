...
void make_mesh(int n)
{
	...
	fprintf(fp, "pigment { color red 0.8 green 0.8 blue 0.8 transmit 0.6} finish { ambient 0.1 diffuse 0.5 } } plane { <0,-1,0>, 10 pigment {checker color White, color Grey}}\n");
...
{
	uint num_par=0;
	char filename[] = "Partic_Shots/000000.pov";
	char header[] = "#include \"colors.inc\" \n \
					camera { \n \
					location <5,10,-35> \n \
					look_at <0,0,0> \n \
	...
	for(uint count=0; count<sph_system->m_num_particle; count++)
	{
		//if(sph_system->m_host_mem[count].pos.z < sph_system->m_world_length/5)
		//{
		//	continue;
		//}
		...
		/*if(sph_system->m_host_mem[count].energy <= 0.6f)
		{
			fprintf(fp, "sphere { <%f, %f, %f>, %f texture { pigment { color Blue } } }\n", x, y, z, 0.2f);
		}
		else
		{
	...
	fprintf(fp, "plane { <0,-1,0>, 10 pigment {checker color White, color Grey}}\n");
...
void keyboard_func(unsigned char key, int x, int y)
{
	...
	if(key == 'y' || key == 'Y')
	{
		sph_system->m_host_param->force_waiting_left=true;
	}

	if(key == 'u' || key == 'u')
	{
		sph_system->m_host_param->force_waiting_right=true;
	}
...
void menu_key(unsigned char key, int /*x*/, int /*y*/)
{
    switch(key) 
    {
		...
		case '2':
			...
			printf("Enable Split: %u\n", sph_system->m_use_split);
			break;
		case '3': 
			sph_system->m_use_merge=1-sph_system->m_use_merge;
			printf("Enable Merge: %u\n", sph_system->m_use_split);
			break;
		case '4': 
