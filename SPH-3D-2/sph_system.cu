...
void integrate_velocity_kernel(Particle* dev_mem, uint num_particle)
{
	...
	{
		if(p->pos.x < dev_param.world_width/3 && p->pos.y > dev_param.world_height/5)
		{
					...
					uint *dev_status)
{
	...
	{        
		...
		while(num_merge > 0)
		{
			...
			dev_mem[p1st].pos=(dev_mem[p1st].pos*V1+dev_mem[p2nd].pos*V2)/(V1+V2);
			dev_mem[p1st].vel=(dev_mem[p1st].vel*V1+dev_mem[p2nd].vel*V2)/(V1+V2);
			dev_mem[p1st].ev=(dev_mem[p1st].ev*V1+dev_mem[p2nd].ev*V2)/(V1+V2);
			dev_mem[p1st].acc=(dev_mem[p1st].acc*V1+dev_mem[p2nd].acc*V2)/(V1+V2);
